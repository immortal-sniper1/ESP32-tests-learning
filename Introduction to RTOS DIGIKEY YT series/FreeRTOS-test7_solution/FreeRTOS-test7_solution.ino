...
static SemaphoreHandle_t serial_and_read;     // mutex for serial and reading from buffer
static SemaphoreHandle_t sem_params; // Counts free space in buffer 
...
void consumer(void *parameters)
{
  ...
  while (1) {
    ...
    if (xSemaphoreTake(serial_and_read, 0) == pdTRUE)
    {
      ...
      xSemaphoreGive(serial_and_read);
...
void setup()
{
  ...
  // Create mutex for serial and buffer
  serial_and_read = xSemaphoreCreateMutex();
  xSemaphoreTake(serial_and_read, 0);
  //counter for buffer (initialize at buffer size-1)
  sem_params = xSemaphoreCreateCounting(num_tasks, BUF_SIZE);
